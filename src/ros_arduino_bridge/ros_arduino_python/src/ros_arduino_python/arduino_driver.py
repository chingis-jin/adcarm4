...
class Arduino:
    ...
    def drive(self, leftF, leftB,rightF,rightB):
    ...
    def drive_m_per_s(self, leftF,leftB,rightF,rightB):
        ''' Set the motor speeds in meters per second.
        '''
        leftF_revs_per_second = float(leftF) / (self.wheel_diameter * PI)
        rightF_revs_per_second = float(rightB) / (self.wheel_diameter * PI)
        leftB_revs_per_second = float(leftF) / (self.wheel_diameter * PI)
        rightB_revs_per_second = float(rightB) / (self.wheel_diameter * PI)

        leftF_ticks_per_loop = int(leftF_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        rightF_ticks_per_loop  = int(rightF_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        leftB_ticks_per_loop = int(leftB_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        rightB_ticks_per_loop  = int(rightB_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)

        self.drive(leftF_ticks_per_loop , leftB_ticks_per_loop, rightF_ticks_per_loop,rightB_ticks_per_loop)
